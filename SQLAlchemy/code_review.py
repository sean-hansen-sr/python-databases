...
def get_coin_prices(coins, currencies):
    ...
    data = requests.get(COINGECKO_URL, verify=False).json()
    return data
...
#engine = create_engine("postgresql://postgres:password@localhost/manager")
...
def view_portfolio():
    with Session(engine) as session:
        ...
        for index, portfolio in  enumerate(all_portfolios):
            print(f"{index + 1}: {portfolio.name} - {portfolio.description}")
        portfolio_index = int(input("Select portfolio by number: ")) - 1
        ...
        print(f"Investments in portfolio '{portfolio.name}':")
        for index, investment in enumerate(investments):
            coin_price = coin_prices[investment.coin][investment.currency.lower()]
            total_value = coin_price * float(investment.amount)
            print(f"{index + 1}: {investment.coin} {total_value:,.2f} {investment.currency})")
        print("Prices provided by CoinGecko")

@click.command(help="Create a new investment and add it to a portfolio")
@click.option("--coin", prompt="Coin (e.g., bitcoin)", help="The cryptocurrency to invest in")
@click.option("--currency", prompt="Currency (e.g., usd)", help="The currency to use for the investment")
@click.option("--amount", prompt="Amount", type=float, help="The amount to invest")
def add_investment(coin: str, currency: str, amount: float):
    with Session(engine).no_autoflush as session:
        ...
        portfolio_index = int(input("Select portfolio by number: ")) - 1
        portfolio = all_portfolios[portfolio_index]
        investment = Investment(coin=coin, currency=currency, amount=amount, portfolio=portfolio)
        portfolio.investments.append(investment)
        session.add(investment)
        session.commit()
        print(f"Added investment of {amount} {coin} in {currency} to portfolio '{portfolio.name}'.")

@click.command(help="Create a new portfolio")
@click.option("--name", prompt="Portfolio name", help="The name of the portfolio")
@click.option("--description", prompt="Portfolio description", help="A brief description of the portfolio")
def add_portfolio(name: str, description: str):
